...
class AccountsControllerTest {
  ...
  void transferMoney() throws Exception {
    ...
    this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
            .content("{\"accountId\":\"Id-456\",\"balance\":50}")).andExpect(status().isCreated());
    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
            .content("{\"accountFromId\":\"Id-123\",\"accountToId\":\"Id-456\",\"amount\":50}")).andExpect(status().isOk());
    ...
    assertThat(toAccount.getBalance()).isEqualByComparingTo("100");
  }
